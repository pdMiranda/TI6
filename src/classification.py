...
def create_model(image_size, num_classes):
    """
    Cria o modelo CNN usando Transfer Learning com base no ResNet50.
    """
    base_net = ResNet50(
        weights='imagenet',
        include_top=False,
...
def plot_and_save_history(history, output_dir):
    """
    Plota e salva as curvas de acurácia e perda do treinamento em um diretório específico.
    """
...
if __name__ == "__main__":
    # --- 1. Definição de Parâmetros e Caminhos ---
    script_dir = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.dirname(script_dir)
    BASE_DIR = os.path.join(project_root, 'dataset')
    TRAINING_DIR = os.path.join(BASE_DIR, 'Training')
    TESTING_DIR = os.path.join(BASE_DIR, 'Testing')
    OUTPUT_DIR = os.path.join(BASE_DIR, 'plots') # Diretório para salvar os gráficos
    ...
    # Salva os gráficos de perda e acurácia no diretório de saída
    plot_and_save_history(history, OUTPUT_DIR)
    print(f"Gráficos de treinamento foram salvos em: {OUTPUT_DIR}")
    ...
    # Cria e salva a matriz de confusão no diretório de saída
    os.makedirs(OUTPUT_DIR, exist_ok=True)
